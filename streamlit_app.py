...
# ===== Fuzzy DPJP =====
# Token gelar/derajat yang tidak menentukan identitas orang
STOP_TOKENS = {
    "DR", "DRG", "SP", "B", "M", "K",
...
def _norm_doctor(s: str) -> str:
    ...
    s = s.upper()
    # non-huruf -> spasi agar token tidak nempel
    s = re.sub(r"[^A-Z]+", " ", s)
    # seragamkan BMM/BM jadi token terpisah (selaras dengan penulisan bertitik)
...
def _score_doctor(raw: str, canon: str):
    ...
    if not ta or not tb:
        return 0.0, 0
    # fokus nama (buang gelar)
    na, nb = ta - STOP_TOKENS, tb - STOP_TOKENS
    inter_n = na & nb
    sn = (len(inter_n) / len(na | nb)) if (na and nb) else 0.0  # skor nama
    sa = len(ta & tb) / len(ta | tb)                            # skor umum
    return 0.85 * sn + 0.15 * sa, len(inter_n)                  # bobot: nama 85%

def map_doctor_to_canonical(raw: str, candidates=DPJP_CANON, threshold: float = 0.35) -> str:
    best, best_score = "", 0.0
    for c in candidates:
        sc, inter_name_cnt = _score_doctor(raw, c)
        # wajib ada minimal 1 token nama yg sama (tajrin, gazali, irfan, dll.)
        if inter_name_cnt == 0:
            continue
        if sc > best_score:
...
def build_block(no, row, visit_key, base_date):
    ...
    # APPLY lowercasing 'drg.' untuk hasil edit manual di tabel
...
def parse_pdf_to_rows_and_period_bytes(pdf_bytes: bytes):
    ...
    for i, m in enumerate(matches):
        ...
        # APPLY lowercasing 'drg.' untuk hasil mapping otomatis
